$(document).ready(()=> {
    ...
    $.get({
        ...
        success: function(response) {
            if (response === true) {
                setNavbarToggler();
                toggleUsers('login');
                doAjax('/dashboard', 'GET', main, main, $('.errorWelcomePageNotFetched').text());
                navbar_brand.show();
    ...
    function notifyError(text, jqXHR) {
        ...
        $('.modal-overlay-message .modal-content').empty().append(`
            ...
            <div class="messageErrorStatus" style="font-size:16px;">${$('.errorCode').text()} ${JSON.parse(jqXHR.responseText).status}</div>
            <div class="messageErrorMessage" style="font-size:16px;">${$('.errorMessage').text()}<br/>${JSON.parse(jqXHR.responseText).message}</div>`);
    }
        ...
        setTimeout(function() {
            $.post({
                ...
                error: function(jqXHR) {
                    closeWait();
                    main.delay(450).fadeIn();
                    notifyError($('.error' + text + 'Failed').text(), jqXHR);
                }
            });
        }, 450);
    }
    ...
    function toggleUsers(text) {
        if (text === 'register' || text === 'login') {
            doAjax('/userNavbar', 'GET', $('.navbarUsers'), $('.navbarUsers'), $('.errorNavbarLoadingFailed').text());
    ...
    function renderRightNavLogin() {
        ...
        doAjax('/rightNavLogin', 'GET', $('.rightNavButtons'), $('.rightNavButtons'), $('.errorNavbarLoadingFailed').text());
    }

    // Adjusts right navbar’s section after logout
    function renderRightNavLogout() {
        $('.rightNavButtons').fadeOut();
        doAjax('/rightNavLogout', 'GET', $('.rightNavButtons'), $('.rightNavButtons'), $('.errorNavbarLoadingFailed').text());
    }

    // Adjusts navbar after logout
    function renderNavbarLogout() {
        doAjax('/mainNavbarLogout', 'GET', $('.navbarUsers'), $('.navbarUsers'), $('.errorNavbarLoadingFailed').text());
        $('.navbarUsers').fadeOut();
    }
    ...
    $(document).on('click', '.validateBlockchain', function() {
        openWait();
        doAjax('/validateBlockchain', 'GET', main, main, $('.errorBlockchainNotValidated').text());
    ...
    $(document).on('click', '[name="jumbotronLogin"], .navLogin', ()=> {
        ...
        setNavbarToggler();
        doAjaxLogin('/login', 'GET', main, main, $('.errorLoginPageNotFetched').text());
    ...
    $(document).on('click', '[name="register"], .navRegister', ()=> {
        ...
        setNavbarToggler();
        doAjax('/register', 'GET', main, main, $('.errorRegisterPageNotFetched').text());
        ...
        $.get({
            ...
            success: function(response) {
                if (response === true) {
                    ...
                    setTimeout(()=> {
                        doAjaxWelcome('/welcome', 'GET', main, main, $('.errorWelcomePageNotFetched').text());
