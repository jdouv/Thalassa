...
<div class="contract bg-text container animate slideIn vw-100">
    <section class="row contractTitleGroup">
        <div class="col-lg-4 offset-lg-4 mr-auto align-self-center contractTitle"><spring:message code="legal.contracts.titles.upper.${composedContract['contract'].title}"/></div>
        <div class="col-lg-auto contractTitleQRGroup">
            <div class="contractQR qrCode" title="<spring:message code="basics.copyQR"/>"></div>
            <div class="copiedQR text-center" style="display:none;"><spring:message code="basics.copiedQR"/></div>
            <div class="contractHash qrCodeText" style="display:none;">${composedContract["hash"]}</div>
            <div class="contractTimestamp">${composedContract["timestamp"]}</div>
        </div>
    </section>
    <div class="row">
        <div class="col">
            <button class="collapseButton" type="button" data-toggle="collapse" data-target="#contractingParties" aria-expanded="false" aria-controls="contractingParties"><spring:message code="legal.contractingParties"/></button>
                        ...
                        <c:forEach items="${composedContract['contract'].essentials['signatures']}" var="signature">
                            <div class="col-auto infoBox contractSignature">
                                <div class="signatureQRGroup">
                                    <div class="contractSignaturesQR qrCode" title="<spring:message code="basics.copyQR"/>"></div>
                                    <div class="copiedQR text-center" style="display:none;"><spring:message code="basics.copiedQR"/></div>
                                    ...
                                    <div class="signatureValidation" style="display:none;">
                                        ...
                                        <div class="copiedQR"><spring:message code="blockchain.signatureValid"/></div>
                                    </div>
                                    <div class="signatureValidation" style="display:none;">
                                        <div class="symbol bgTextSymbolCenter errorTextColor signatureValidationSymbol"></div>
                                        <div class="copiedQR"><spring:message code="blockchain.signatureInvalid"/></div>
                                    </div>
                                </div>
                                <div class="rightInfo">
                                    <div><span class="bold signatureNameHighlight">${signature["signer"].firstName} ${signature["signer"].lastName}</span></div>
                                    <div><span class="bold"><spring:message code="users.positions.${CaseUtils.toCamelCase(signature['signer'].position, false)}"/></span></div>
                                    <c:choose>
                                        <c:when test="${signature['onBehalfOf'] != null}">
                                            <div><spring:message code="legal.contracts.onBehalfOf"/></div>
            ...
            <button class="collapseButton" type="button" data-toggle="collapse" data-target="#essentials" aria-expanded="false" aria-controls="essentials"><spring:message code="legal.contracts.essentials"/></button>
                        ...
                        <c:when test="${composedContract['contract'].essentials['type'] == 'timeCharter'}">
                            ...
                            <div class="row justify-content-center essentialsData">
                                <div class="col-auto infoBox">
                                    ...
                                    <div class="rightInfo">
                                        <div><span class="bold"><spring:message code="vessels.name"/>:</span> ${composedContract["contract"].essentials["vessel"].name}</div>
                                        <div><span class="bold"><spring:message code="vessels.imoNumber"/>:</span> ${composedContract["contract"].essentials["vessel"].imoNumber}</div>
                                        <div><span class="bold"><spring:message code="vessels.flag"/>:</span> ${composedContract["contract"].essentials["vessel"].flag}</div>
                                        <div><span class="bold"><spring:message code="vessels.yearBuilt"/>:</span> ${composedContract["contract"].essentials["vessel"].yearBuilt}</div>
                                        <div><span class="bold"><spring:message code="vessels.dwt"/>:</span> ${composedContract["contract"].essentials["vessel"].dwt} M/T</div>
                                    </div>
                                </div>
                            </div>
                        </c:when>
        ...
        <div class="col">
            <c:choose>
                <c:when test="${not empty composedContract['contract'].clauses}">
                    <button class="collapseButton" type="button" data-toggle="collapse" data-target="#clauses" aria-expanded="false" aria-controls="clauses"><spring:message code="legal.contracts.clauses"/></button>
                        ...
                        <div class="card card-body">
                            <c:forEach items="${composedContract['contract'].clauses}" var="clause">
                                <div class="contractClause">
                                    ...
                                    <div class="row" title="<spring:message code="legal.contracts.toggleLegen"/>">
                                        <div class="col paragraphReadable mr-lg-3">
                                            <div class="clauseSubtitle animate slideIn" style="display:none;"><spring:message code="legal.contracts.readable"/></div>
                                    ...
                                    <div style="text-align:center;margin-top:10px;"><button type="button" class="button enableClause"><spring:message code="legal.enableClause"/></button></div>
                                </div>
                            </c:forEach>
                        </div>
                    </div>
